...
def fit_models(X_train, y_train, X_test, y_test, n):
    ...
    y_tr = y_train
    y_t = y_test
    X_train.to_csv(f'tio2/{n}/X_train.csv', index=True, sep=';')
    y_train.to_csv(f'tio2/{n}/y_train.csv', index=True, sep=';')
    ...
    precision_kfold = precision_score(y_train, y_pred_kfold)
    recall_kfold = recall_score(y_train, y_pred_kfold)
    f1_kfold = f1_score(y_train, y_pred_kfold)
    ...
    precision_ext = precision_score(y_test, y_pred_ext)
    recall_ext = recall_score(y_test, y_pred_ext)
    f1_ext = f1_score(y_test, y_pred_ext)
    ...
    y_pred_ext_prob = model.predict_proba(X_test)[:, 1]
    fpr_ext, tpr_ext, _ = roc_curve(y_test, y_pred_ext_prob)
    ...
    plt.title(f'tio2/{n}/ROC-AUC Curve (k-fold)')
    plt.legend(loc="lower right")
    plt.savefig(f'tio2/{n}/roc_auc_kfold.png')
    plt.close()
    ...
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title(f'tio2/{n}/ROC-AUC Curve (External)')
    plt.legend(loc="lower right")
    plt.savefig(f'tio2/{n}/roc_auc_ext.png')
    plt.close()

    # Save the model to a file
    filename = f"tio2/{n}/model.pkl"
    ...
    plt.savefig(f'tio2/{n}/confusion_matrix_kfold.png')
    plt.close(fig)

    fig, ax = plt.subplots(dpi=300)
    disp_ext = ConfusionMatrixDisplay(confusion_ext)
    disp_ext.plot(ax=ax)
    plt.savefig(f'tio2/{n}/confusion_matrix_ext.png')
    plt.close(fig) 
    ...
    importance_df.to_csv(f'tio2/{n}/f_importance.csv', index=True, sep=';')
...
for n in range(1, 15):
    ...
    y_train = Y_train[[n]]
    y_test = Y_test[[n]]
    res = fit_models(X_train, y_train, X_test, y_test, n)
...
result_models.to_csv('tio2/results.csv', index=True, sep=';')